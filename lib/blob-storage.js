...
const { BLOB_STORAGE }                                = require('../utils/config');
...
async function blobStore() {
  if (BLOB_STORAGE === 'tmpfile') {
    await shellCommand(`mkdir -p tmp/blobs`);
    return fsBlobStore('tmp/blobs');
  } else {
...
module.exports = {
  blobStore, encryptToBlobStore, writeToBlobStream, readFromBlobStream, decryptBlob
};
